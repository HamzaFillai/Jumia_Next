...
app.get("/getCluster/:id",(request,response)=>{
    db.query("SELECT * FROM `tutorials_cluster` WHERE idUser=? and (classe, date) IN (SELECT classe, Max(date) FROM `tutorials_cluster` GROUP BY classe) ORDER BY date LIMIT 4",[request.params.id],(err,result)=>{
        if(err){
            console.log(err);
        }else{
